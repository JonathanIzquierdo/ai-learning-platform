...
export default function App() {
        ...
        <nav className="max-w-6xl mx-auto flex items-center justify-between px-4 py-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setView('home')}
              className="px-3 py-1 rounded-full text-sm font-medium text-slate-400 hover:text-white transition-all">
              {lang === 'es' ? 'Cursos' : 'Courses'}
            </button>
            <span className="px-3 py-1 rounded-full text-sm font-medium bg-blue-600 text-white">
              {COMPASS} {lang === 'es' ? 'Comparador de Modelos' : 'Model Advisor'}
            </span>
            <LanguageSwitcher />
          </div>
        </nav>
  ...
  return (
    <div className="min-h-screen bg-slate-900 px-4 py-8">
      <nav className="max-w-5xl mx-auto flex items-center justify-between mb-12">
        <div className="flex items-center gap-3">
          <img src={VISMA_LOGO} alt="Visma" className="h-7 w-auto" />
          <span className="text-xs font-bold text-slate-500 tracking-widest uppercase">LATAM</span>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => setView('advisor')}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white border border-slate-700 hover:border-slate-600 transition-all">
            {COMPASS} <span className="hidden sm:inline">{lang === 'es' ? 'Comparador de Modelos' : 'AI Model Advisor'}</span><span className="sm:hidden">Advisor</span>
          </button>
          <LanguageSwitcher />
        </div>
      </nav>
