...
## Org Constraints — READ THIS BEFORE SUGGESTING ARCHITECTURAL CHANGES

Jona is a developer at Visma LATAM but **does NOT have access to Visma corporate infrastructure**:

- **No access to Visma Google Cloud Console** (workspace `visma.com`). Anything requiring OAuth clients in the corporate workspace (e.g., "Sign in with Google" tied to `hd=visma.com`) needs an IT ticket and is slow/blocked.
- **No access to Visma DNS** (`visma.com` zone). Cannot add SPF/DKIM/MX records on the corporate domain. Cannot create subdomains (`learning.visma.com`, etc.) without IT.
- **No corporate budget access** for paid SaaS without procurement.

**Implications for solutions:**

- Auth must stay magic-link via Supabase (no Google OAuth on `visma.com` without IT).
- SMTP/email infrastructure uses Jona's **personal domain `pixails.pro`** (registered on MonsterASP) — NOT `visma.com`.
- Workarounds use Jona's personal infra wherever possible. Anything that requires the corp domain → ticket → weeks of waiting.

When proposing changes, default to solutions that work without IT involvement. If a corporate-dependency change is the right long-term answer, flag it as "blocked on IT ticket" rather than treating it as a normal next step.
...
- **Resend** — transactional email (SMTP relay for magic links), domain `pixails.pro`
...
### Email / SMTP — LIVE (configured 2026-05-21)

Supabase Auth sends magic-link emails via **Resend** (configured under Authentication → Emails → SMTP Settings).

- **Provider:** Resend (https://resend.com)
- **Verified sending domain:** `pixails.pro` (Jona's personal domain; NOT `visma.com` — see Org Constraints)
- **Sender email:** `noreply@pixails.pro`
- **Sender name:** `Visma AI Learning`
- **DNS records** live in MonsterASP DNS panel (https://admin.monsterasp.net/), zone `pixails.pro`:
  - `TXT  resend._domainkey   p=MIGfMA0G...QAB`   ← DKIM (verification)
  - `MX   send                feedback-smtp.sa-east-1.amazonses.com  priority 10`
  - `TXT  send                v=spf1 include:amazonses.com ~all`
  - (DMARC at `_dmarc` is optional, not configured)
- **DNS subdomain choice:** Records are on `send.pixails.pro` (and `resend._domainkey.pixails.pro`), NOT on the root. This isolates Resend from any future mail setup on the apex domain and means changes to email infra don't affect the existing `pixails.pro` site.

**Operational notes:**

- Free tier of Resend (3,000 emails/month) covers Visma LATAM volume comfortably.
- Magic links deliver to any `@visma.com` recipient now that the domain is verified.
- If email breaks, check in this order: (1) Supabase auth-logs (https://supabase.com/dashboard/project/wwzrqduocihohbuvitdq/logs/auth-logs) for the actual error message, (2) Resend dashboard for delivery/bounce events, (3) DNS record status in Resend (must show "Verified").
- DO NOT change the Sender email to `@visma.com` — that would require corporate DNS access (see Org Constraints).
...
These bit us during the auth rollout, admin panel build, and SMTP setup. Save yourself the time.
...
10. **Resend sandbox mode only delivers to the account owner.** Without a verified sending domain, Resend rejects any recipient that isn't the email tied to the Resend account. The error surfaces in Supabase as a generic 500 / `unexpected_failure` on POST `/auth/v1/otp`. The actual Resend message is buried in `auth-logs`: `gomail: could not send email 1: 550 You can only send testing emails to your own email address`. Diagnosis flow: app shows "Error sending magic link email" → check Supabase auth-logs for the SMTP-level error → if it mentions Resend test-mode restrictions, the fix is verifying a domain (NOT scaling up SMTP credentials, NOT changing Supabase config).

11. **Visma corporate network intercepts/rewrites DNS queries.** Running `nslookup foo.pixails.pro` from a corp machine can return SOA records for `visma.com` instead — the corp resolver is doing split-horizon or filtering. This makes local DNS debugging useless for non-Visma domains. To check propagation honestly: use `nslookup foo.pixails.pro 8.8.8.8` (force Google DNS), use https://dnschecker.org, or just rely on Resend's "Verify DNS Records" button (Resend resolves against authoritative nameservers directly and isn't affected by Visma's resolver).

12. **DKIM TXT values are long but go in a single TXT record.** ~250+ character base64 strings. Most DNS panels accept this fine, but some old ones split at 255 chars and want multiple quoted strings. MonsterASP took the value as-is without issues. If a panel rejects "value too long", split into chunks of ≤255 chars each, quoted, on the same record.
...
- **Supabase Auth → SMTP Settings** — host/port/user/password are Resend credentials. Only the Sender email/name should be edited, and only if you understand the domain-verification implications.
- **`pixails.pro` DNS records `send` and `resend._domainkey`** in MonsterASP — deleting these breaks all magic-link delivery.
...
## External Services (out-of-band, not MCP)

- **Supabase dashboard** — https://supabase.com/dashboard/project/wwzrqduocihohbuvitdq
- **Resend dashboard** — https://resend.com (account on Jona's personal email; domain `pixails.pro`)
- **MonsterASP DNS panel** — https://admin.monsterasp.net (DNS for `pixails.pro`)
- **Vercel dashboard** — Vercel project for `ai-learning-platform-livid.vercel.app`
...
- Brand the Resend magic-link email template with Visma logo/colors (currently uses Supabase default)

### Done (recent)

- 2026-05-21 — Resend SMTP live with `pixails.pro` domain verified. Magic links now deliver to any `@visma.com` (previously only Jona's own address worked due to Resend sandbox mode).
- 2026-05-21 — Admin panel (`/admin`) shipped with Dashboard, Users, Events tabs.
- 2026-05-21 — Migration 005 added (FKs for PostgREST nested selects).
...
- **Email volume.** Resend free tier = 3,000 emails/month. Magic links + password resets stay well under this for Visma LATAM volume. If usage grows past free tier, paid Resend plans start at ~$20/mo for 50k.

---

## Last Updated

2026-05-21 — Resend SMTP went live with `pixails.pro` as verified sending domain. Email-related "Lessons Learned" entries #10–12 added (Resend sandbox, Visma corp DNS interception, DKIM TXT length). New "Org Constraints" section added at the top — Jona has no access to Visma corporate Google Cloud or DNS, so all infrastructure uses personal resources. Admin panel + migration 005 still current. Spanish quiz review still pending push.
